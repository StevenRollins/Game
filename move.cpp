...
void do_ranged_combat(dungeon *d, character *atk, pair_t target,
 bool explosive)
{
	uint32_t damage, i;
	damage = 0;
...
const char *attacks[] = {
	...
	if (explosive)
	{
	for (int x = target[dim_x-1]; x < target[dim_x+1]; x++){
	for (int y = target[dim_y-1]; y < target[dim_y+1]; y++)
	{
		if (x >= 1 && x < 80 && y >= 1 && y < DUNGEON_Y)
		{
			...
			damage = atk->damage->roll();
		       io_queue_message("%s%s %s your %s for %d.", is_unique(atk) ? "" : "The ",
        		...
        		if (i == eq_slot_weapon && !d->PC->eq[i]) {
      			...
      			io_queue_message("You hit %s%s for %d.", is_unique(d->character_map[target[dim_y]][target[dim_x]]) ? "" : "the ",
                       	d->character_map[y][x]->name, damage);
    			}     if (damage >= d->character_map[y][x]->hp) {
      if (atk != d->PC) {
        ...
        io_queue_message("%s%s dies.", is_unique(d->character_map[y][x]) ? "" : "The ", d->character_map[y][x]->name);
      }
      d->character_map[y][x]->hp = 0;
      d->character_map[y][x]->alive = 0;
      character_increment_dkills(atk);
      character_increment_ikills(atk, (character_get_dkills(d->character_map[y][x]) +
                                       character_get_ikills(d->character_map[y][x])));
      if (d->character_map[y][x] != d->PC) {
        d->num_monsters--;
      }
      d->character_map[y][x] = NULL;
    } else {
      d->character_map[y][x]->hp -= damage;
    }
			...
			{
				if (damage < d->hardness[y][x])
				{
					d->hardness[y][x] = d->hardness[y][x]-damage;
				}
				else
				{
					d->hardness[y][x] = 0;
	...
	{
		if (d->character_map[target[dim_y]][target[dim_x]])
		{
			if (charpair(target) == d->PC)
			{
			damage = atk->damage->roll();
		       io_queue_message("%s%s %s your %s for %d.", is_unique(atk) ? "" : "The ",
                       atk->name, attacks[rand() % (sizeof (attacks) /
        		...
        		if (i == eq_slot_ranged && !d->PC->eq[i]) {
      			...
      			io_queue_message("You shoot %s%s for %d.", is_unique(d->character_map[target[dim_y]][target[dim_x]]) ? "" : "the ",
                       	d->character_map[target[dim_y]][target[dim_x]]->name, damage);
    			}     if (damage >= d->character_map[target[dim_y]][target[dim_x]]->hp) {
      if (atk != d->PC) {
        io_queue_message("You die.");
        io_queue_message("As %s%s eats your %s,", is_unique(atk) ? "" : "the ",
                         atk->name, organs[rand() % (sizeof (organs) /
        ...
        io_queue_message("%s%s dies.", is_unique(d->character_map[target[dim_y]][target[dim_x]]) ? "" : "The ", d->character_map[target[dim_y]][target[dim_x]]->name);
      }
      d->character_map[target[dim_y]][target[dim_x]]->hp = 0;
      d->character_map[target[dim_y]][target[dim_x]]->alive = 0;
      character_increment_dkills(atk);
      character_increment_ikills(atk, (character_get_dkills(d->character_map[target[dim_y]][target[dim_x]]) +
                                       character_get_ikills(d->character_map[target[dim_y]][target[dim_x]])));
      if (d->character_map[target[dim_y]][target[dim_x]] != d->PC) {
        d->num_monsters--;
      }
      d->character_map[target[dim_y]][target[dim_x]] = NULL;
    } else {
      d->character_map[target[dim_y]][target[dim_x]]->hp -= damage;
    }
			

			}
		else if(d->map[target[dim_y]][target[dim_x]])
		{
			if (damage < d->hardness[target[dim_y]][target[dim_x]])
			{
				d->hardness[target[dim_y]][target[dim_x]] = d->hardness[target[dim_y]][target[dim_x]]-damage;
			}
			else
			{
				d->hardness[target[dim_y]][target[dim_x]] = 0;
				d->map[target[dim_y]][target[dim_x]] = ter_floor_hall;
			}
		}
	}
