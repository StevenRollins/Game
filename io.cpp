...
static uint32_t io_display_obj_info(object *o)
{
  ...
  mvprintw(1, 0, " %-80s ", s);
  mvprintw(3, 0, " %-80s ", o->get_description());
...
static uint32_t io_inspect_monster(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(0, 0,"%-80s",s);
  mvprintw(2, 0, "%-80s" ,((npc *) charpair(dest))->description);
...
uint32_t io_aim(dungeon *d)
{
  ...
  mvprintw(0, 0, "Choose a target.  'g' or '.' to select; 'ESC' to cancel.");
  ...
  * not otherwise used.                                                 */
  ...
  if (c == 'g' || c == '.') {
	if (d->PC->eq[2]){
		do_ranged_combat(d, d->PC, dest, false);
		return 0;
	}
	else
	{
...
void io_build_wall(dungeon *d)
{
	int c;
	
	uint32_t x = d->PC->position[dim_x];
	uint32_t y = d->PC->position[dim_y];
	int i = 0;
	io_queue_message("use number keys to place walls 5 to cancle");
	...
	{
	case '1':
		if(d->map[y+1][x-1] == ter_floor_room || d->map[y+1][x-1] == ter_floor_hall)
		{
			d->map[y+1][x-1] = ter_wall;
			d->hardness[y+1][x-1] = 40;
			io_queue_message("you place wall");
			io_display(d);
			i = 1;
		}
	break;
	case '2':
		if(d->map[y+1][x] == ter_floor_room || d->map[y+1][x] == ter_floor_hall)
		{
			d->map[y+1][x] = ter_wall;
			d->hardness[y+1][x] = 40;
			io_queue_message("you place wall");
			i = 1;
			io_display(d);	
	...
	break;
	case '4':
		if(d->map[y][x-1] == ter_floor_room || d->map[y][x-1] == ter_floor_hall)
		{
			d->map[y][x-1] = ter_wall;
			d->hardness[y][x-1] = 40;
			io_queue_message("you place wall");
			i = 1;
			io_display(d);	
